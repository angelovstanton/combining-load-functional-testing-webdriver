...
namespace E2E.Load.Core
{
    public class TestScenarioExecutor
    {
        ...
        private readonly RestClient _apiClientService;
        private readonly Dictionary<string, string> _scenarioCookies;

        public TestScenarioExecutor(List<LoadTestAssertionHandler> loadTestAssertionHandlers)
        {
            _loadTestAssertionHandlers = loadTestAssertionHandlers;
            _scenarioCookies = new Dictionary<string, string>();
            _apiClientService = new RestClient();
            _apiClientService.AddHandler("application/json", () => NewtonsoftJsonSerializer.Default);
            _apiClientService.AddHandler("text/json", () => NewtonsoftJsonSerializer.Default);
            _apiClientService.AddHandler("text/x-json", () => NewtonsoftJsonSerializer.Default);
            _apiClientService.AddHandler("text/javascript", () => NewtonsoftJsonSerializer.Default);
            _apiClientService.AddHandler("*+json", () => NewtonsoftJsonSerializer.Default);
        ...
        private IMeasuredResponse ExecuteMeasuredRequest(IRestRequest request)
        {
            var watch = Stopwatch.StartNew();
            IRestResponse response = _apiClientService.Execute(request);
            watch.Stop();
